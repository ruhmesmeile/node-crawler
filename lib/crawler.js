...
exports.Crawler = function(options) {
    ...
    self.request = function(opts) {
        ...
        if (opts.uri) {
            var onlyHead = _.find(bins, function (item) {
                return opts.uri.toLowerCase().indexOf(item) === opts.uri.length - item.length;
            });
        ...
        var req = request(_.pick.apply(this,[ropts].concat(requestArgs)), function(error,response,body) {
            ...
            //dont follow 404 - hack
            if (response.statusCode == '404') {
                response.body = '';
                return self.onContent(error, opts, response);
            }
    ...
    self.onContent = function (error, toQueue, response, fromCache) {
        ...
        if (toQueue.forceUTF8) {
            ...
            if (!toQueue.incomingEncoding) {
                ...
                if (iconv) {
                    var iconvObj = new iconv(toQueue.incomingEncoding, "UTF-8//TRANSLIT//IGNORE");
                    response.body = iconvObj.convert(response.body).toString();
                    // iconv-lite doesn't support Big5 (yet)
                } else if (toQueue.incomingEncoding != "Big5") {
                    response.body = iconvLite.decode(response.body, toQueue.incomingEncoding);
                }
            }
                    ...
                    var env = jsdom.env({
                        ...
                        "done":function(errors,window) {
                            ...
                            try {
                                ...
                                toQueue.callback(errors,response,window.jQuery);
                            } catch (e) {
                                callbackError = e;
                            }
